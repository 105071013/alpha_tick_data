...
Original file is located at
    https://colab.research.google.com/drive/1xUa9s_bXA3UcAKa2BlTARKI0BTdea9TO
"""
...
import io
import pandas as pd
data_train = pd.read_csv(io.BytesIO(uploaded1['train_3406.csv']))

uploaded2 = files.upload()
data_test = pd.read_csv(io.BytesIO(uploaded2['test_3406.csv']))
...
#3 drop out layer
#y用秒來分 (10秒)
model_2 = lstm_stock_model(X_train.shape)
#callback = EarlyStopping(monitor="val_mean_squared_error", patience=10, verbose=1, mode="min")
history_2 = model_2.fit(X_train, Y_train, epochs=2, batch_size=batch_size[0], validation_split=validation_split[2], shuffle=True)#, callbacks=[callback], shuffle=True)

X_t2 = model_2.predict(X_train)
Xt2 = model_2.predict(X_test)
X_t2 = X_t2.reshape(1, len(X_t2))[0]
Xt2 = Xt2.reshape(1, len(Xt2))[0]
X_t2 = true_range(X_in, X_t2, 'return_10s')
Xt2 = true_range(X_out, Xt2, 'return_10s')

print('train set')
model_performance(X_in['return_10s'].iloc[10:].values, X_t2)
print('test set')
model_performance(X_out['return_10s'].iloc[10:].values, Xt2)

plt.plot(X_t2)

plt.plot(Xt2)

train_return_10 = pd.DataFrame(X_t2)
test_return_10 = pd.DataFrame(Xt2)
train_return_10.to_csv('train_3406_mix1.csv') 
files.download('train_3406_mix1.csv')

test_return_10.to_csv('test_3406_mix1.csv') 
files.download('test_3406_mix1.csv')
...
#3 drop out layer
#y用秒來分 (10秒)
model_3 = lstm_stock_model(X_train.shape)
#callback = EarlyStopping(monitor="val_mean_squared_error", patience=10, verbose=1, mode="min")
history_3 = model_3.fit(X_train, Y_train, epochs=1, batch_size=batch_size[0], validation_split=validation_split[2], shuffle=True)#, callbacks=[callback], shuffle=True)

X_t3 = model_3.predict(X_train)
Xt3 = model_3.predict(X_test)
X_t3 = X_t3.reshape(1, len(X_t3))[0]
Xt3 = Xt3.reshape(1, len(Xt3))[0]
X_t3 = true_range(X_in, X_t3, 'return_10s')
Xt3 = true_range(X_out, Xt3, 'return_10s')

print('train set')
model_performance(X_in['return_10s'].iloc[10:].values, X_t3)
print('test set')
model_performance(X_out['return_10s'].iloc[10:].values, Xt3)
